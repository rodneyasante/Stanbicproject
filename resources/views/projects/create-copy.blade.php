					...
					<form method="post" action="{{route('projects.store')}}" enctype="multipart/form-data">
						...
						<div class="modal-body">					
							...
							<div class="form-group">
								...
								<input type="date" class="form-control" required name="end_date" min="{{ date('Y-m-d') }}" value="{{old('end_date')}}">
							</div>
							<div class="form-group">
							...
							<div class="form-group">
								<p>Do you want to create url from template</p>					
							  	<select class="form-control" id="create_template" value="{{old('template')}}">
						      		<option value=1>Yes</option>
						      		<option value=0>No</option>
						    	</select>
						    	<div id="selection" style="{{old('template') ? '' : 'display:none'}}">
									<label>If no enter URL</label>
									<input type="text" class="form-control" placeholder="Leave blank for no URL" v-model="currentEvent.url">
								</div>
							</div>
						</div>
